...
export default function AdminPage() {
  ...
  useEffect(() => {
    return () => {
      if (selected?.carePermissionPreviewUrl) {
        URL.revokeObjectURL(selected.carePermissionPreviewUrl);
      }
    };
  }, [selected?.carePermissionPreviewUrl]);
  ...
  async function openCarePermission(caregiver) {
    const response = await axios.get(`/api/admin/caregivers/${caregiver.id}/care-permission`, { headers, responseType: 'blob' });
    const previewUrl = URL.createObjectURL(response.data);
    window.open(previewUrl, '_blank', 'noopener,noreferrer');
    setSelected((current) => {
      if (current?.carePermissionPreviewUrl) {
        URL.revokeObjectURL(current.carePermissionPreviewUrl);
      }
      return current?.id === caregiver.id ? { ...current, carePermissionPreviewUrl: previewUrl } : current;
    });
  }
