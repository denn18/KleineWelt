      ...
      <form className="grid gap-5" onSubmit={handleSubmit}>
        ...
        <section className="grid gap-4 rounded-2xl border border-brand-100 bg-white/80 p-4">
          ...
          <div className="rounded-2xl border border-brand-100 bg-brand-50/60 p-3">
            ...
            <div className="mt-2 inline-flex rounded-full border border-brand-200 bg-white p-1">
              <button
                type="button"
          ...
          {formState.isLargeDaycare ? (
            <div className="grid gap-2 rounded-2xl border border-sky-200 bg-sky-50/70 p-3">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-xs font-semibold text-sky-800">Betreiber der Großkindertagespflege</h3>
                <button
                  type="button"
                  onClick={handleAddLargeDaycareOperator}
                  className="rounded-full border border-sky-300 bg-white px-2 py-1 text-[11px] font-semibold text-sky-700"
                >
                  + Betreiber
                </button>
              </div>
              {formState.largeDaycareOperators.map((operatorName, index) => (
                <div key={`mobile-large-daycare-operator-${index}`} className="flex items-center gap-2">
                  <input
                    value={operatorName}
                    onChange={(event) => handleLargeDaycareOperatorChange(index, event.target.value)}
                    placeholder={`Betreiber ${index + 1}`}
                    className="w-full rounded-xl border border-sky-200 bg-white px-3 py-2 text-sm shadow-sm focus:border-sky-400 focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => handleRemoveLargeDaycareOperator(index)}
                    className="rounded-full border border-slate-200 bg-white px-2 py-2 text-[11px] font-semibold text-slate-600"
                  >
                    ✕
                  </button>
                </div>
