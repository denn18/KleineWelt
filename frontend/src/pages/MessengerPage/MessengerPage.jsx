...
function MessengerPage() {
  ...
  useEffect(() => {
    if (!isMobile || loadingMessages) {
      return undefined;
    }

    const timeoutId = window.setTimeout(() => {
      mobileBottomRef.current?.scrollIntoView({ behavior: 'auto', block: 'end' });
    }, 30);

    return () => window.clearTimeout(timeoutId);
  }, [isMobile, loadingMessages, messages.length]);
                          ...
                          {(message.attachments || []).length ? (
                            <div className="mt-2 space-y-2">
                              {message.attachments.map((attachment) => {
                                ...
                                return (
                                  <div key={key} className={`flex items-center gap-3 rounded-xl border px-3 py-2 text-left ${attachmentClasses}`}>
                                    {isImage ? (
                                      <button
                                        type="button"
